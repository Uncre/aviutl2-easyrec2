...
本プラグインは、aoytsk氏が公開しているAviUtl 1.x用プラグイン[「簡易録音」配布ページ](https://aoytsk.blog.jp/aviutl/1677824.html)の「プレビューを再生しながら録音する」「録音後に拡張編集のタイムラインへ自動追加する」という使い勝手を参考に、AviUtl 2でも同等の基本的な録音体験を実現することを目標として制作しました。

旧 `rec.auf` のソースコードを利用した移植・改変や逆コンパイルではなく、公開されているAviUtl ExEdit2 Plugin SDKを使用した新規実装です。また、旧版作者による公式な後継版ではありません。

EasyRec2では基本的な録音体験を引き継ぎながら、WAV以外の音声形式、保存先の明示指定、録音時間表示、実時間どおりのタイムライン挿入、録音機器の互換フォールバックなどを追加しています。
